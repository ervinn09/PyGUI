function [fsNew, fileLengthNew] = pcmUp2_v02(pcmInPath, pcmOutName, fileLength, fs, ii)
...
% filter
[status, result] = system(['filter -up SHQ2 ' tmp_up_1 ' ' tmp_up_2 ' ' num2str(filterLength)]);
if status ~= 0
    disp(result)
    error('Something went wrong');
end
